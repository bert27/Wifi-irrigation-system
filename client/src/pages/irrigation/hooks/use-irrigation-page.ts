...
export const useIrrigationPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setErrorGet(`${t('irrigation.errors.connection')}: ${error}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const createTask = async () => {
    ...
    if (selectedDays.length === 0) {
      setErrorCreateTask(t('irrigation.errors.noDaySelected'));
      return;
    }
    ...
    try {
      ...
      setErrorCreateTask(t('irrigation.errors.saveFailed'));
    }
  };
