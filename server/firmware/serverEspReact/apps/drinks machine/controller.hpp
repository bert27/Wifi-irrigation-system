...
public:
    ...
    void begin() {
        ...
        // Subscribe to remote control events for menu navigation
    ...
    void processRotation(bool forward) {
        if (!insideMenuDrink) {
            actualScreen = 0;
            if (forward) {
                counter++;
    ...
    void handleRemoteCommand(const struct_message& msg) {
        ...
        if (joyDir == "Arriba") {
            processRotation(false); // Scroll up
        } else if (joyDir == "Abajo") {
            processRotation(true);  // Scroll down
        }

        if (String(msg.joystickValues.buttonState) == "on") {
            clickButton();
        }
    }
...
private:
    ...
    void handleEncoderRotation() {
        ...
        if (aState != aLastState) {
            processRotation(digitalRead(PIN_DT) != aState);
        }
        aLastState = aState;
    }
};
